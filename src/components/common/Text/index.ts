...
) => styled.span<TypoProps>`
  font-size: ${(props) => {
    const fontSizeGroup = props.theme.typography.fontSize[variant] as Record<
      string,
      string
    >;
    ...
    return (
      fontSizeGroup[props.size] ||
      fontSizeGroup["md"] ||
      fontSizeGroup["default"]
    );
  }};

  font-weight: ${(props) =>
    props.theme.typography.fontWeight[props.weight] || "medium"};
`;
